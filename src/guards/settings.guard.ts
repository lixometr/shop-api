...
export class SettingsGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        let locale = await this.localeService.findBySlug({ slug: localeSlug }, new RequestPayload({ request: req }))
        if (!locale) {
        ...
        let currency = await this.currencyService.findBySlug({ slug: currencySlug }, new RequestPayload({ request: req }))
        if (!currency) {
